...
The mean and median are both estimators for the center of a distribution, but they behave differently when data are skewed. For symmetric distributions they give similar values (for the Normal distribution they are both _unbiased_ and _consistent_ estimators of $\mu$), but for right-skewed data the mean is pulled toward the long tail while the median stays closer to the bulk of the data.
...
Suppose we have an experiment with _n_ = 5. In statistics, we describe that an estimator, such as the sample mean, is unbiased if:
...
## Key R functions for replication
