...
Use the slider to choose a sample size `n`, then press "Draw Sample" to draw a new random sample of `n` penguins from the Gentoo population and compute the sample mean. The red line marks the sample mean; the dashed green line marks the true population mean.
...
//| echo: false
viewof controls = {
  const s = Inputs.range([5, 100], {value: 30, step: 1, label: "Sample size n"});
  const t = Inputs.toggle({label: "High-definition", value: false});
  const div = html`<div style="display:flex;align-items:center;gap:2rem">${s}${t}</div>`;
  div.value = {n: s.value, hd: t.value};
  s.oninput = t.oninput = () => {
    div.value = {n: s.value, hd: t.value};
...
viewof draw_btn = {
  ...
  btn.style = "padding:4px 14px;margin-top:0.3rem";
  btn.value = JSON.stringify({n: 30, hd: false});
  btn.onclick = () => {
    btn.value = JSON.stringify({n: n_slider, hd: hd});
    btn.dispatchEvent(new CustomEvent("input", {bubbles: true}));
  };
  return btn;
}
draw_n = JSON.parse(draw_btn).n
draw_hd = JSON.parse(draw_btn).hd
```
...
#|   - draw_n
#|   - draw_hd
sample_mass <- sample(gentoo_mass, size = draw_n, replace = TRUE)
plot_penguin_sample(gentoo_mass, sample_mass, draw_hd)
```
