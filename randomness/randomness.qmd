...
//| echo: false
{
  ...
  function drawPenguin(x, y, facingRight, r) {
    const bs = r * 0.9;
    const cx = x, cy = y - r;
    const bx = cx + (facingRight ? r * 0.85 : -r * 0.85);
    const by = cy;
    ctx.beginPath();
...
As you see, when $P$ finally reaches the peak, all $P$ finds up there is a small $\omega$. $P$ picks it up and carries it back down. $P$ then repeats this again and again, but can never grab the $\Omega$.

Then $P$ comes to a realization. As $P$ collects all of the $\omega$s from $\Omega$ mountain, $P$ realizes they all have different weight. This is exactly how the expectation works: random datasets are sampled in real life proportional to their probability. When we define the expectation using integration, we have to weigh each dataset according to how likely it is to be seen. $\Omega$ represents all possible datasets, an infinite amount. Only through calculus can we tame infinity and resolve to a finite sum.
...
points(px - 0.6, py - 0.2, pch = 17,
       col = adjustcolor("darkorange", 0.5), cex = 2.5)
```
...
So the expectation is a 5-dimensional integral: sum up the sample mean over every conceivable dataset, each weighted by how probable that dataset is under the data-generating process. This is exactly what the simulations in this document approximate: each replicate is one $\omega$, and averaging $\bar{x}_{n=5}(\omega)$ across many replicates estimates the integral. 
