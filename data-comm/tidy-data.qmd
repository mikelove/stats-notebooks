...
We start from the complete cases of the `penguins` dataset, then simulate a realistic missing-not-at-random (MNAR) mechanism: penguins heavier than 5250 g have their body mass value suppressed 75% of the time. A companion indicator column records whether each value is missing.
...
penguins_obs <- penguins_complete |>
  mutate(
    body_mass_missing = body_mass_g > 5250 & runif(n()) < 0.75,
    body_mass_obs = if_else(body_mass_missing, NA, body_mass_g)
  )
```

Note the difference in the body mass for the missing penguins (as expected). To show this we introduce `slice_sample()` which allows for random sampling within groups. 
...
Sometimes the values that are missing are correlated with other variables that we *do* observe. Flipper length is highly correlated with body mass in penguins. And our mechanism of missingness in this case (by construction) has to do with extreme values of body mass, so even when `body_mass_obs` is `NA` we still have `flipper_length_mm` which may tell us about the expectation of missingness. Let's check on this correlation using our full dataset. This is analagous to knowing such a correlation between variables exists from literature.
...
Again, this correlation implies we can model the binary missingness indicator as a function of flipper length using logistic regression. Logistic regression is implemented in R via `glm()` with `family = binomial`. Technically the model is a Bernoulli regression — a special case of the binomial where the number of trials is 1 — and the response must be in {0, 1} (or equivalently `TRUE`/`FALSE`). Like `lm()`, it fits a linear combination of predictor variables — here to the log-odds of the response being 1 — but instead of minimizing squared residuals, it estimates coefficients by maximizing the likelihood of the observed 0/1 outcomes under the model. The fitted log-odds can be converted to a probability with `type = "response"` in `predict()`. The question we are asking: does a longer flipper predict a higher probability of the body mass being missing?

::: {.callout-note collapse="false"}
## Exercise

Fit a logistic regression predicting `body_mass_missing` from `flipper_length_mm` in `penguins_obs`.
:::
...
The coefficient on `flipper_length_mm` is positive and significant: penguins with longer flippers (i.e. heavier ones) are indeed more likely to have their body mass missing, which recovers the MNAR mechanism we built in. We can now attach the model's predicted missingness probability back to each row with `mutate()`:
...
Plotting `prob_missing` against true body mass, with point shape indicating whether the value was actually suppressed, lets us see how well the model separates the two groups:
...
The open circles (missing values) are concentrated at high body mass, but some have a low predicted probability of missingness. Why is the prediction of missingness not simply increasing with body mass?
:::
